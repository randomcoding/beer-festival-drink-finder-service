...
class WbfSpreadsheetLoaderTest extends FunSuite with ShouldMatchers {
	...
	test("Correct Ciders are loaded ") {
		...
		matched.find(_.name.equals("Orchard Bull")).get should be(cider("Orchard Bull", "", 6.5, 0.0, List(DrinkFeature("Medium Dry"))))
		matched.find(_.name.equals("Orchard Bull")).get.brewer should be(Brewer("Ashgrove"))
		matched.find(_.name.equals("Orchard Harvest")).get should be(cider("Orchard Harvest", "", 6.0, 0.0, List(DrinkFeature("Medium Sweet"))))
		matched.find(_.name.equals("Orchard Harvest")).get.brewer should be(Brewer("Ashgrove"))
		matched.find(_.name.equals("Rum Cask")).get should be(cider("Rum Cask", "", 7.3, 0.0, List(DrinkFeature("Medium"))))
		matched.find(_.name.equals("Rum Cask")).get.brewer should be(Brewer("Barbourne"))
		matched.find(_.name.equals("Standard Orchard Blend")).get should be(cider("Standard Orchard Blend", "", 6.0, 0.0, List(DrinkFeature("Dry"))))
	...
	test("Correct Perries are loaded") {
		...
		matched.find(_.name.equals("Barland")).get should be (perry("Barland", "", 7.400000000000001, 0.0, Nil))
		matched.find(_.name.equals("Barland")).get.brewer should be (Brewer("Barbourne"))
		matched.find(_.name.equals("B.U.R.P.")).get should be (perry("B.U.R.P.", "", 6.1, 0.0, Nil))
		matched.find(_.name.equals("B.U.R.P.")).get.brewer should be (Brewer("Barkers"))
		matched.find(_.name.equals("Blakeney Red")).get should be (perry("Blakeney Red", "", 6.5, 0.0, Nil))
