...
class ChappelSpreadsheetLoaderTest extends FunSuite with ShouldMatchers {
	...
	test("Load Beer Data from sample spreadsheet with loaded source") {
		...
		drink.features should be (List(DrinkFeature("Resto")))
	}

	
	test("Correct Beers are loaded") {
		// Commented out drinks are in All Gone state and therefore not returned from the match.
		...
		drink  should be (beer("CHORISTER", "", 4.5, 0.0, List(DrinkFeature("Resto"))))
		drink.quantityRemaining should be ("Plenty")
		drink.brewer should be (Brewer("ABBEY ALES"))
		
		drink = matched.find(_.name.equals("KLETSWATER")).get
		drink  should be (beer("KLETSWATER", "", 4.0, 0.0, List(DrinkFeature("Goods"))))
		...
		drink should be (beer("ON THE RAILS", "", 3.8, 0.0, List(DrinkFeature("Resto"))))
		...
		drink should be (beer("DUNSTABLE GIANT", "", 4.4, 0.0, List(DrinkFeature("Resto"))))
		drink.quantityRemaining should be ("Plenty")
		drink.brewer should be (Brewer("B&T"))
		
		drink = matched.find(_.name.equals("EDWIN TAYLOR'S STOUT")).get
		drink should be (beer("EDWIN TAYLOR'S STOUT", "", 4.5, 0.0, List(DrinkFeature("Resto"))))
		drink.quantityRemaining should be ("Plenty")
		drink.brewer should be (Brewer("B&T"))
		
		drink = matched.find(_.name.equals("SHEFFORD DARK MILD")).get
		drink should be (beer("SHEFFORD DARK MILD", "", 3.8, 0.0, List(DrinkFeature("Resto"))))
		drink.quantityRemaining should be ("Plenty")
		drink.brewer should be (Brewer("B&T"))
		
		/*drink = matched.find(_.name.equals("SOS")).get
		drink should be (beer("SOS", "", 5.0, 0.0, List(DrinkFeature("Resto"))))
		drink.quantityRemaining should be ("All Gone")
		drink.brewer should be (Brewer("B&T"))
		
		drink = matched.find(_.name.equals("BEST BITTER")).get
		drink should be (beer("BEST BITTER", "", 4.2, 0.0, List(DrinkFeature("Resto"))))
		drink.quantityRemaining should be ("All Gone")
		drink.brewer should be (Brewer("BALLARDS"))*/
		
		drink = matched.find(_.name.equals("GOLDEN BINE")).get
		drink should be (beer("GOLDEN BINE", "", 3.8, 0.0, List(DrinkFeature("Resto"))))
		drink.quantityRemaining should be ("Plenty")
		drink.brewer should be (Brewer("BALLARDS"))
	}
}